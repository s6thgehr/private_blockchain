...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      const prevHeight = await self.getChainHeight();

      block.height = prevHeight + 1;
      block.time = new Date().getTime().toString().slice(0, -3);
      if (prevHeight >= 0) {
        const lastBlock = await self.getBlockByHeight(prevHeight);
        block.previousBlockHash = lastBlock.hash;
      }

      block.hash = SHA256(JSON.stringify(block)).toString();
      let errors = await self.validateChain();
      console.log(`Errors from validating: ${errors}`);
      if (errors.length === 0) {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      console.log(`Time difference: ${currentTime - messageTime}`);
